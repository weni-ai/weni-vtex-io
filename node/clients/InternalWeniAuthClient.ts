...
export class InternalWeniAuthClient extends ExternalClient {
  ...
  private async getToken(): Promise<string> {
    ...
    while (attempt < retries) {
      try {
        ...
        if (attempt >= retries) {
          ...
          console.error(
            `Failed to fetch access token after ${retries} attempts: ${err.message}`
          )
          throw new Error(
            `Failed to fetch access token after ${retries} attempts.`
          )
        }
      }
    ...
    console.error(
      'Unexpected error: failed to fetch access token and reached an unreachable state.'
    )
    throw new Error(
      'Unexpected error: failed to fetch access token and reached an unreachable state.'
    )
