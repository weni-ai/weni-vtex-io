...
export async function proxyAbandonedCartNotification(ctx: ServiceContext<Clients>, next: () => Promise<any>) {
  try {
    ...
    const { cart_id, account, phone } = requestBody
    if (!cart_id || !account || !phone) {
      ctx.status = 400
      ctx.body = { message: 'Missing required fields in request body - required: cart_id, account, phone', body: requestBody }
      return
    }
