 ...
 */
export async function proxyWhatsAppIntegration(
  ...
  try {
    ...
    if (!project_uuid || !waba_id || !phone_number_id || !auth_code) {
      ...
      ctx.body = {
        message:
          'Missing required fields: project_uuid, waba_id, phone_number_id, auth_code.',
        error:
          'Missing required fields: project_uuid, waba_id, phone_number_id, auth_code.',
      }
    ...
    const response = await integrationEngineClient.createWhatsAppIntegration(
      requestBody,
